...
**Wichtiger Hinweis zu Sonderzeichen im Passwort:** Separate Credentials-Dateien (`/root/.smbcreds`) haben Probleme mit bestimmten Sonderzeichen wie `!` – der `mount.cifs`-Parser interpretiert diese Zeichen unzuverlässig und lehnt die Authentifizierung mit `STATUS_LOGON_FAILURE` ab, selbst wenn das Passwort korrekt ist. Deshalb schreiben wir die Credentials **direkt ins Script** mit **einfachen Anführungszeichen**, die verhindern dass die Shell Sonderzeichen interpretiert.
...
# Credentials inline, da Sonderzeichen im Passwort Probleme mit separater Credentials-Datei machen
...
for i in $(seq 1 $MAX_TRIES); do
    ...
    if mount -t cifs //192.168.178.3/Proxmox "$MOUNT_POINT" \
        -o 'username=proxmox,password=DEIN_PASSWORT_HIER,uid=0,gid=0,vers=3.0,iocharset=utf8' 2>/dev/null; then
...
Speichern. **`DEIN_PASSWORT_HIER` durch das echte NAS-Passwort ersetzen**, die einfachen Anführungszeichen um die gesamten Mount-Optionen müssen bleiben.
...
Das Script hat nicht die Ausführungsrechte. Prüfen:

```bash
ls -la /opt/scripts/mount-nas.sh
```

Sollte `-rwx------` (700) sein. Falls `-rw-------` (600), fix:
...
1. **Sonderzeichen im Passwort** (besonders `!`): Sorgen bei separaten Credentials-Dateien (`/root/.smbcreds`) für Parser-Probleme. Lösung: Credentials direkt ins Script mit einfachen Anführungszeichen um die gesamten Mount-Optionen packen (siehe Abschnitt 4.2).
...
Manuell testen mit Credentials direkt auf der Kommandozeile:

```bash
mount -t cifs //192.168.178.3/Proxmox /tmp/testmount -o 'username=proxmox,password=DEIN_PASSWORT,uid=0,gid=0,vers=3.0,iocharset=utf8' -v
```

Die **einfachen Anführungszeichen** sind wichtig – sie verhindern dass Bash Sonderzeichen wie `!` interpretiert.
